...
class MemberCommitmentPaymentModal extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final selectedAccountId =
        store.selectedAvailabilityAccountId ?? accounts.first.availabilityAccountId;
    ...
    return Column(
      ...
      children: [
        ...
        Input(
          ...
          initialValue:
              store.paidAt != null
                  ? formatDateToDDMMYYYY(store.paidAt!)
                  : '',
          readOnly: true,
          onChanged: (_) {},
          onTap: () async {
          ...
          iconRight: const Icon(Icons.calendar_today, color: AppColors.purple),
        ),
    ...
    if (account.configurations?.enableBankSlip ?? false) {
      channels.add(
        _ChannelCard(
          ...
          description: l10n.member_contribution_payment_method_boleto_description,
        ),
      );
    }
    channels.add(
