...
class PatrimonyAssetFormState {
  ...
  factory PatrimonyAssetFormState.fromModel(PatrimonyAssetModel asset) {
    ...
    return PatrimonyAssetFormState(
      ...
      existingAttachments: List<PatrimonyAttachmentModel>.from(
        asset.attachments,
      ),
      attachmentsToRemove: {},
    );
  ...
  FormData toFormData({bool includeImmutableFields = true}) {
    ...
    if (acquisitionDate.isNotEmpty) {
      formData.fields.add(
        MapEntry('acquisitionDate', convertDateFormat(acquisitionDate)),
      );
    }
    ...
    formData.fields.add(
      MapEntry('category', category ?? PatrimonyAssetCategory.other.apiValue),
    );
